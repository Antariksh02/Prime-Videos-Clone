...
export default function SinglePage() {
  ...
  const watchdata = useSelector((state)=>state.movies.mymovie);
  const dd = watchdata?.map((value)=>value.id)
  ...
  const notifyNew = () => toast.success('Added to Watchlist');
  const alreadyHave = () => toast.success('Already in Watchlist');
  const handleAddToWatchlist = (data) => {
    dispatch(addMovie(data));
  
    if (dd.includes(data.id)) {
      alreadyHave();
    } else {
      notifyNew();
    }
...
<h6>Trailer</h6>
  ...
  <button onClick={()=>handleAddToWatchlist(data)} style={{border:"none",borderRadius:"50%",padding:"5px", background:"black",color:"white",marginLeft:"10px"}}><AddIcon/></button>
